...
SYMPTOM_OPTIONS = [
    "Body Pain", "Head Hurting", "Dizziness", "Emotional Dysregulation", "Leg Pains",
    "Cramps", "Stomach Pain", "Headache", "Bloating", "Fatigue", "Breast Tenderness",
    "Mood Swings", "Anxiety", "Insomnia", "Cravings", "Back Pain"
]
...
def convert_to_datetime(df, column_name):
    ...
    try:
        ...
        if df[column_name].isnull().any():
            raise ValueError(f"Some values in '{column_name}' could not be converted to datetime.")
...
def add_symptoms(df):
    ...
    for idx, row in df.iterrows():
        ...
        while True:
            ...
            if custom_symptoms:
                selected_symptoms.extend([symptom.strip() for symptom in custom_symptoms.split(',') if symptom.strip()])
            ...
            if not st.checkbox("Add another entry for this cycle?", key=f"add_entry_{idx}_{len(symptom_entries)}"):
                break
...
def visualize_symptom_frequency_and_severity(df):
    ...
    symptom_df = pd.DataFrame([
        {'Symptom': symptom, 'Count': data['count'], 'Avg Severity': data['total_severity'] / data['count']}
        for symptom, data in symptom_data.items()
    ])
    
    fig = px.bar(symptom_df, x='Symptom', y='Count',
                 title='Symptom Frequency and Average Severity Across Cycles',
                 labels={'Count': 'Frequency', 'Symptom': 'Symptom Type'},
                 color='Avg Severity', color_continuous_scale='RdYlBu_r',
                 hover_data=['Avg Severity'])

    fig.update_layout(xaxis_tickangle=-45, xaxis_title='Symptom', yaxis_title='Frequency')
...
def visualize_cycle_length(df):
    ...
    - None
    '''
    fig = px.line(df, x='Start Date (dd/mm/yyy)', y='Cycle Duration (days)',
                  title='Cycle Length Over Time',
                  labels={'Start Date (dd/mm/yyy)': 'Cycle Start Date', 'Cycle Duration (days)': 'Cycle Length (days)'},
                  markers=True)
...
def visualize_symptom_heatmap(df):
    ...
    - None
    '''
    all_symptoms = sorted(set(sym for symptom_dict in df['Symptom Data'] for date_symptoms in symptom_dict.values() for sym in date_symptoms.keys()))
    ...
    for idx, symptom_dict in enumerate(df['Symptom Data']):
        ...
        for symptom in all_symptoms:
            symptom_matrix.at[idx, symptom] = np.mean(cycle_symptoms[symptom]) if cycle_symptoms[symptom] else np.nan
...
def pattern_recognition(df):
    ...
    patterns['most_common_symptoms'] = sorted(all_symptoms.items(), key=lambda x: x[1]['count'], reverse=True)[:5]

    # Find most severe symptoms
    patterns['most_severe_symptoms'] = sorted(all_symptoms.items(), key=lambda x: x[1]['total_severity'] / x[1]['count'], reverse=True)[:5]
    ...
    patterns['symptom_correlations'] = sorted(correlation_counts.items(), key=lambda x: x[1], reverse=True)[:5]
...
def generate_recommendations(patterns, knowledge_base):
    ...
    for symptom, data in patterns['most_severe_symptoms'][:3]:
        if symptom in knowledge_base and knowledge_base[symptom][0] not in recommendations:
...
def main():
    st.title("Menstrual Cycle Tracker with Date-Specific Symptom Tracking and Confidence Measures")

    uploaded_file = st.file_uploader("Upload your menstrual cycle data (Excel file)", type=["xlsx"])
    ...
    if uploaded_file:
        try:
            ...
            cycle_df, avg_cycle_length, std_cycle_length, next_cycle_prediction, prediction_interval, coefficient_of_variation = calculate_cycle_data(df)
            ...
            st.write(f"**Standard Deviation of Cycle Length:** {std_cycle_length:.2f} days")
            
            if next_cycle_prediction:
                st.write(f"**Next Cycle Prediction:** {next_cycle_prediction.date()}")
                st.write(f"**95% Prediction Interval:** {prediction_interval[0].date()} to {prediction_interval[1].date()}")
                st.write(f"**Coefficient of Variation:** {coefficient_of_variation:.2f}%")
                
                st.write("### Interpretation of Results:")
                st.write(f"We are 95% confident that the next cycle will start between {prediction_interval[0].date()} and {prediction_interval[1].date()}.")
                
                if coefficient_of_variation < 10:
                    st.write("Your cycles are considered regular (Coefficient of Variation < 10%).")
                elif 10 <= coefficient_of_variation < 15:
                    st.write("Your cycles show some variability (10% ≤ Coefficient of Variation < 15%).")
                else:
                    st.write("Your cycles show high variability (Coefficient of Variation ≥ 15%). This may affect the accuracy of predictions.")
            else:
                st.write("Not enough data to make predictions or calculate confidence measures.")
